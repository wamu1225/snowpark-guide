...
export const nullsEntries: Entry[] = [
  {
    ...
    pitfall:
      '**report原文には「整数型の列に浮動小数点数でfill_nullすると型エラーになる」とあるが、Polars 1.43.2で実際に試したところエラーにはならなかった**。整数列（`i64`）に`fill_null(0.0)`のような浮動小数点数を渡すと、**例外を出さずに列全体の型がこっそり`f64`へ変わる**（実行して確認済み）。「エラーで気づく」のではなく「型が静かに変わる」ため、後続の処理で誤差や想定外の桁が出てから気づくことになりやすい。整数列を保ちたいときは、埋める値の型（`0`か`0.0`か）を明示的に意識する。',
    snowparkDocUrl:
